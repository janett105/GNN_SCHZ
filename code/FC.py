...
def create_FC(subj_dir, subj_id,error_subjects,
              dtr=False, stdz=False, smth = None, cnfstdz = None, confs=["motion","scrub", "wm_csf","global_signal"], connstdz = "zscore_sample",
    ...
    try : 
        confdata = pd.read_csv(f'{subj_dir}/{subj_id}/func/{subj_id}_task-rest_desc-confounds_timeseries.tsv', sep='\t')

        # sample mask : 조건(scrubbing) 따라 제거된 volume 없는 경우 None, 있는 경우 제거되지 않은 volumes index
    ...
    if nomat:
        ...
        viz_correlation_matrix(FC_df, atlas.labels)
...
def makeFC(subj_dir, dataset_name):
    ...
    for subj_id in subj_list:
        ...
        try :
            print(f'==============={subj_id}=======================')
            FC_df, time_series, reginparc = create_FC (subj_dir, subj_id, error_subjects,
                            dtr=False, stdz=False, smth = None, cnfstdz = None, confs=["motion","scrub", "wm_csf","global_signal"], connstdz = "zscore_sample",
                            conntype='correlation', space='data', savets=False, nomat=False)
...
makeFC(subj_dir, dataset_name) # Atlas : Schaefer2018_100parcels_7networks_1mm