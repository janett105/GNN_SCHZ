...
for class_weights in param_grid['class_weights']:
    ...
    for n_fold, (train_val, test) in enumerate(skf.split(labels, labels)):  
        ...
        for epoch in range(n_epoch):
            ...
            val_sen, val_spe, val_bac, v_loss = GCN_test(model, optimizer, val_loader, class_weights, len(val_dataset), n_fold, epoch, device)

            if np.isnan(val_sen): 
                isnan=True
                break

            test_sen, test_spe, test_bac, _ = GCN_test(model, optimizer, test_loader, class_weights, len(val_dataset), n_fold, epoch, device)
