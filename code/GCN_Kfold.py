...
def GCN_Kfold(dataset, labels, batch, param_grid, skf, 
    ...
    for class_weights in param_grid['class_weights']:
        ...
        train_met = np.zeros((n_splits, n_metrics))
        historys_loss=[]
        historys_sen=[]
        historys_spe=[]
        historys_bac=[]
        ...
        for n_fold, (train_val, test) in enumerate(skf.split(labels, labels)):  
            ...
            for epoch in range(n_epoch):
                ...
                val_sen, val_spe, val_bac, v_loss = GCN_test(model, optimizer, val_loader, class_weights, len(val_dataset), n_fold, epoch, device, savefiglog)

                if np.isnan(val_sen): 
                    isnan=True
                    break

                test_sen, test_spe, test_bac, _ = GCN_test(model, optimizer, test_loader, class_weights, len(val_dataset), n_fold, epoch, device, savefiglog)

                if savefiglog: print(f'train loss : {t_loss} , val loss : {v_loss}')
            ...
            train_met[n_fold, 0] = best_train_sen
            train_met[n_fold, 1] = best_train_spe
            train_met[n_fold, 2] = best_train_bac
